### Filter Amino Acid Variants 3 (FAAV3)
### This file filters the FAAV2 SNP datasets to get all mutations that have at least one disease associated with a protein mutation.
...
logging.info(f'This file filters the FAAV2 SNP datasets to get all mutations that have at least one disease associated with a protein mutation.')
...
# Filter for variants with protein alleles associated with disease
            ...
            if any(alleleAnnot.get('clinical')):
                for assemblyAnnot in alleleAnnot.get('assembly_annotation'):
                    for gene in assemblyAnnot.get('genes'):
                        for protein_mut in gene.get('rnas'):
                            if protein_mut.get('protein', {}):
                                entries.append(entry)
                                filteredSNPs += 1  # Increment the filtered count
                                for disease_entry in alleleAnnot.get('clinical'):
                                    for sig_type in disease_entry.get('clinical_significances'):
                                        diseaseSig[sig_type] += 1
                                found = True
                                break
                        if found:
                            break
